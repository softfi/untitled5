...
Future<LoginResponse> logInApi(Map request,
  ...
  debugPrint("945076899867 ${isSocialLogin}");
  if (!(response.statusCode >= 200 && response.statusCode <= 206)) {
    if (response.body.isJson()) {
      var json = jsonDecode(response.body);
      debugPrint("945076899867 ${json}");
      if (json.containsKey('code') &&
          json['code'].toString().contains('invalid_username')) {
        throw 'invalid_username';
      }
  ...
  return await handleResponse(response).then((json) async {
    ...
    if (loginResponse.data != null) {
      ...
      await sharedPref.setString(
          ADDRESS, loginResponse.data!.address.validate());
      print("loginResponse.data" + loginResponse.data!.id.toString());
...
Future<LoginResponse> getUserDetail({int? userId}) async {
  debugPrint("  kjwfiuwefiwuegfjwkqehf $userId");
...
Future<NearestGHospitalModel> getNearHospitalByLatLng(
    {String? lat, String? lng}) async {
  return NearestGHospitalModel.fromJson(await handleResponse(
      await buildHttpResponse('near-by-hospital?latitude=$lat&longitude=$lng',
          method: HttpMethod.GET)));
}
...
Future<void> updateNewHiteshTip({required Map request}) async {
   LDBaseResponse.fromJson(await handleResponse(await buildHttpResponse(
      'update-tips',
      method: HttpMethod.POST,
      request: request)));
}
...
logOutSuccess() async {
  ...
  if (sharedPref.getString(LOGIN_TYPE) == LoginTypeGoogle ||
      !(sharedPref.getBool(REMEMBER_ME) ?? false)) {
