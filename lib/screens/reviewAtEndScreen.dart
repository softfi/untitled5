...
class ReviewAtEndScreen extends StatefulWidget {
  // final Driver? driverData;
  // final OnRideRequest rideRequest;

  ReviewAtEndScreen({key});

  @override
  ReviewAtEndScreenState createState() => ReviewAtEndScreenState();
}

class ReviewAtEndScreenState extends State<ReviewAtEndScreen> {
  ...
  Future<void> userReviewData() async {
    if (true/*formKey.currentState!.validate()*/) {
      ...
      Map req = {
        "ride_request_id":RIDE_REQUEST_ID_BY_ALOK ,
        "rating": rattingData,
                  ...
                  Row(
                    ...
                    children: [
                      ClipRRect(
                        ...
                        child: commonCachedNetworkImage(DRIVER_DATA_BY_ALOK!.profileImage.validate(), height: 60, width: 60),
                      ),
                      SizedBox(width: 16),
                      Column(
                        crossAxisAlignment: CrossAxisAlignment.start,
                        children: [
                          SizedBox(height: 8),
                          Text('${DRIVER_DATA_BY_ALOK!.firstName.validate()} ${DRIVER_DATA_BY_ALOK!.lastName.validate()}', style: boldTextStyle()),
                          SizedBox(height: 4),
                          Text('${DRIVER_DATA_BY_ALOK!.email.validate()}', style: primaryTextStyle()),
                        ],
                      ),
                    ],
                  ),
